...
def get_calibrated_data(
    ...
    checks for cached .npy files, or runs the calibration in memory-mapped batches if a cache miss occurs.
    ...
    if os.path.exists(cache_flat_path) and os.path.exists(cache_dark_path):
        if logger:
            logger.info("🚀 Found cached data! Memory-mapping precomputed arrays instantly...")
        # mmap_mode='r' streams data from disk as needed without exhausting RAM
        return np.load(cache_flat_path, mmap_mode='r'), np.load(cache_dark_path)

    # ---- COMPUTE DATA (Cache Miss) ----
    if logger:
        logger.info("⏳ Cache missing. Executing calibration pipeline in batches...")

    # Load calibration masters from provided paths (2D arrays ~16 MB each)
    ...
    if total_frames == 0:
        raise FileNotFoundError(f"No valid FITS files found in {data_folder}")
    ...
    first_frame_path = fits_files[0]
    with fits.open(first_frame_path) as hdul:
        first_frame = hdul[0].data.astype(np.float32)
    ...
    flat_cor_mmap = np.lib.format.open_memmap(
        cache_flat_path,
        mode='w+',
        dtype=np.float32,
        shape=(total_frames, crop_h, crop_w)
    )

    # Track preview frames for diagnostic grids
    preview_cropped_raw = None
    preview_dark_cor = None

    # ---- BATCH EXECUTION LOOP ----
    if logger:
        logger.info(f"⚙️ Processing {total_frames} frames in batches of {batch_size}...")

    for i in range(0, total_frames, batch_size):
        end_idx = min(i + batch_size, total_frames)
        batch_paths = fits_files[i:end_idx]

        # 1. Load batch FITS frames into RAM
        batch_raw = load_fits_batch(batch_paths)

        # 2. Crop batch
        batch_cropped = crop_images(batch_raw, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, logger=logger)

        # Save preview sample from the first batch for grid plots
        if i == 0:
        ...
        flat_cor_mmap[i:end_idx] = batch_flat_cor
        flat_cor_mmap.flush()

        # Free batch arrays explicitly
        del batch_raw, batch_cropped, batch_dark_cor, batch_flat_cor
        gc.collect()

        if logger:
            logger.info(f"  ✓ Processed frames {i} to {end_idx} / {total_frames}")

    # ---- GENERATE DIAGNOSTIC PREVIEW GRIDS ----
    if preview_cropped_raw is not None:
        plot_image_grid(images=preview_cropped_raw, n_images=n_images_grid, save_name=grid_before_name,
                        plot_image=False)
        del preview_cropped_raw

    if preview_dark_cor is not None:
        plot_image_grid(images=preview_dark_cor, n_images=n_images_grid, save_name=grid_dark_name, plot_image=False)
        del preview_dark_cor

    plot_image_grid(images=flat_cor_mmap[:n_images_grid], n_images=n_images_grid, save_name=grid_flat_name,
                    plot_image=False)

    # Save cropped dark array to cache
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------
    # --- Paths & Directories ---
    FITS_DATA_FOLDER = "sfg/2026-07-10-C6.0/sun_area/SlitJaw"
    FILE_MASTER_FLAT = "MASTER_SAVE/2026-07-10/master_flat.fits"
    FILE_MASTER_DARK = "MASTER_SAVE/2026-07-10/master_dark.fits"
    PARENT_CACHE = "CACHE_DATA"
    DIR_PLOTS = "Plots/2026-07-10"  # Centralized plots folder
