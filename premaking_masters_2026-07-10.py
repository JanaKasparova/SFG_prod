...
def create_flat_animation(flat_folder, idx, save_folder, master_dark=None, fps=15, vmax=None,
                          dpi=200, bitrate=3000, extra_args=None, logger=None):
    """
    Loads flat images in the specified `idx` range, applies dark correction (if provided),
    extracts timestamps, cleans up unnecessary memory, and generates a high-quality MP4 animation.
    """
    ...
    # 1. Pre-allocate NumPy array directly in RAM
    ...
    # Load remaining frames directly into pre-allocated array
    ...
    # Default to high-quality H.264 profile if extra_args isn't specified
    ...
    if logger:
        logger.info(f"--- Generating high-quality animation (DPI={dpi}, Bitrate={bitrate}k): {save_path} ---")

    # 4. Generate animation video with high quality settings
...
def save_data_flat(
    ...
    show_plots=False,
    batch_size=50,
    exclude_idx=None  # Parameter added: (exclude_start, exclude_end)
):
    flat_files = get_fits_filepaths(flat_folder)
    if not flat_files:
        raise FileNotFoundError(f"No FITS files found in {flat_folder}")
    ...
    _, all_intensities = process_fits_in_batches(
        flat_files, master_dark=master_dark, batch_size=batch_size, logger=logger
    )

    # Pass lightweight (N, 1, 1) dummy shape to plotting function to avoid RAM spikes
    plot_average_intensity(
        all_intensities[:, None, None],
        logger=logger,
    ...
    # ---- SLICE USABLE FILES (DROPPING MIDDLE SECTION IF SPECIFIED) ----
    if exclude_idx is not None:
        ex_start, ex_end = exclude_idx
        # Combines the 'x' parts before ex_start and after ex_end
        usable_files = flat_files[idx[0]:ex_start] + flat_files[ex_end:idx[1]]
        print(f"Usable flat count (excluding indices {ex_start}:{ex_end}): {len(usable_files)}")
    else:
        usable_files = flat_files[idx[0]:idx[1]]
    ...
    average_flat, usable_intensities = process_fits_in_batches(
        usable_files, master_dark=master_dark, batch_size=batch_size, logger=logger
    )

    plot_average_intensity(
        usable_intensities[:, None, None],
        logger=logger,
...
if __name__ == "__main__":
    ...
    # =====================================================================
    FLAT_FOLDER = "sfg/2026-07-10-C6.0/sun_centre/SlitJaw"
    DARK_FOLDER = "sfg/2026-07-10-C6.0/sun_dark/SlitJaw"
    PARENT_SAVE_FOLDER = "./MASTER_SAVE"

    BATCH_SIZE = 150
    FLAT_IDX = (0, -1)
    EXCLUDE_FLAT_IDX = (44, 76)  # Drops frames 40 to 64 inclusive
    DARK_VMAX = 1000
    ...
    # High-quality animation rendering options
    ANIMATION_DPI = 200  # Higher resolution (e.g., 200 or 300)
    ANIMATION_BITRATE = 4000  # Higher bitrate in kbps prevents compression artifacts
    ANIMATION_EXTRA_ARGS = ['-vcodec', 'libx264', '-crf', '18', '-pix_fmt', 'yuv420p']



    RUN_FLAT_TESTING = True
    PROCESS_MASTER_DARK = True
    PROCESS_MASTER_FLAT = True
    SHOW_PLOTS = True  # Set to False during heavy runs to save RAM
    # =====================================================================
