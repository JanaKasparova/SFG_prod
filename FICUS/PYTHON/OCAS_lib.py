...
class Measurement:
	...
	def __init__(self, filename, dataset_index=None):
		...
		with h5py.File(filename, "r") as self.of:
			...
			#by default, data = raw_data, but this is toggeled when subtracting dark from light
	...
	def wl_range(self):
		...
		if self.letter == "C":
			_rng = np.load("FICUS/useful_files/WL_range_C.npy")
		elif self.letter == "D":
			_rng = np.load("FICUS/useful_files/WL_range_D.npy")
		else:
